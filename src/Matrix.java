...
class Matrix {
    ...
    Matrix(int rSize, int cSize, SetIntervalContext ic) {
        matrix = new SetInterval[rSize][cSize];
        this.ic = ic;
    }

    Matrix(int rSize, SetIntervalContext ic) {
    ...
    Matrix[] LU() {
        if (this.matrix.length == this.matrix[0].length) {
            ...
            result[0] = new Matrix(this.matrix.length, this.matrix.length, ic);
            result[1] = new Matrix(this.matrix.length, this.matrix.length, ic);
    ...
    Matrix mul(Matrix A) {
        if (this.matrix[0].length == A.matrix.length) {
            ...
            if (A.matrix[0].length == 1) {
                result = new Matrix(this.matrix.length, ic);
                for (int i = 0; i < this.matrix.length; i++) {
    ...
    Matrix mul(SetInterval[] A) {
        if (this.matrix[0].length == A.length) {
            Matrix result = new Matrix(this.matrix.length, ic);
            for (int i = 0; i < this.matrix.length; i++) {
                result.matrix[i][0] = ZERO;
